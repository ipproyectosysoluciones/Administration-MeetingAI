...
- [ ] **TASK-040** `feat: POST /auth/register (org + org-admin)` — creates organization + tenant + admin user + base roles seeding. Tests: happy path, duplicate, tenant bootstrapped correctly.
- [ ] **TASK-041** `feat: POST /auth/login with MFA gate` — Argon2id verify, MFA challenge for enrolled users. Tests: bad password, MFA-required flow, audit event written.

"""Fixtures for organizations-module integration tests (TASK-060, org-CRUD slice).

Mirrors ``tests/integration/users/conftest.py``: real FastAPI app + DB-backed
authorization resolver against the Docker PostgreSQL test database, an ephemeral RS256
keypair, and helpers to register a tenant, mint a super-admin, and add a member.
"""
...
async def create_super_admin(
    ...
    """Insert a platform super-admin and return ``(id, access_token)``."""
    hasher = PasswordHasher()
    async with session_factory() as session:
        user = User(
            email=f"super-{uuid.uuid4()}@example.com",
            password_hash=hasher.hash("superPass123"),
    ...
    return user_id, await mint_token(settings, user_id)
...
async def create_member(
    ...
    *,
    ...
    """Insert a tenant member (Membership + UserRole) and return ``(id, email)``."""

...
- The README badge block (between `badges:start`/`badges:end` markers) is regenerated by `scripts/gen_readme_badges.py`: it runs automatically in the Release workflow, and must be run manually when bumping frontend/backend dependencies.